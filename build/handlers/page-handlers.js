...
function convertToSimplifiedPage(page, markdownContent) {
    return {
        title: page.title,
...
export async function handleGetConfluencePageById(client, args) {
    try {
        if (!args.pageId) {
...
export async function handleGetConfluencePageByName(client, args) {
    try {
        if (!args.title) {
        ...
        if (pages.length === 0) {
            throw new McpError(ErrorCode.InvalidParams, `No pages found with title "${args.title}"`);
        }
        if (pages.length > 1) {
            ...
            throw new McpError(ErrorCode.InvalidParams, `Multiple pages found with title "${args.title}". Please use get_confluence_page_by_id with one of these IDs: ${JSON.stringify(matches)}`);
        }
